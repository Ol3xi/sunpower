...
export default function Hero() {
  ...
  const activeCircuits = circuitPaths.filter((c) =>
    isDay ? !c.isNightOnly : !c.isDayOnly,
  );
  ...
  return (
    <section
      className={`relative overflow-hidden min-h-[90vh] flex items-center transition-colors duration-1000 ${bgClass}`}
    >
      <AnimatePresence>
        {!isDay && (
          <motion.div
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 space-y-8">
            <h1
              className={`text-4xl md:text-5xl font-extrabold tracking-tight leading-[1.1] transition-colors duration-1000 ${textClassTitle}`}
            >
              L'energia che non <br />
              <span className="text-emerald-500">dorme mai.</span>
            </h1>
            <p
              className={`text-lg max-w-lg leading-relaxed transition-colors duration-1000 ${textClassDesc}`}
            >
              Scopri come il nostro sistema gestisce i flussi di energia in
              tempo reale. Clicca sui componenti per i dettagli o cambia momento
              della giornata.
            </p>
            ...
            <div className="inline-flex items-center p-1 bg-slate-200 dark:bg-slate-700 rounded-full shadow-inner">
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-full font-medium transition-all duration-300 ${isDay ? "bg-white text-amber-500 shadow-md" : "text-slate-500 hover:text-slate-700"}`}
              >
                ☀️ Giorno
              </button>
              <button
                onClick={() => setIsDay(false)}
                className={`flex items-center gap-2 px-6 py-3 rounded-full font-medium transition-all duration-300 ${!isDay ? "bg-blue-600 text-white shadow-md" : "text-slate-500 hover:text-slate-700"}`}
              >
                🌙 Notte
              </button>
            </div>
            ...
            <div className="pt-4">
              <button onClick={() => setIsModalOpen(true)} className="px-8 py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-xl shadow-lg shadow-emerald-500/30 transition-all hover:-translate-y-1">
                Richiedi Preventivo
              </button>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 relative">
            ...
            <div className="relative aspect-[4/3] md:aspect-[16/10] w-full rounded-3xl shadow-2xl border border-slate-700/20 bg-slate-200">
              ...
              <div className="absolute inset-0 overflow-hidden rounded-3xl z-0">
                ...
                <div
                  className={`absolute inset-0 bg-blue-950/60 mix-blend-multiply transition-opacity duration-1000 pointer-events-none ${isDay ? "opacity-0" : "opacity-100"}`}
                />
              ...
              <div className="absolute inset-0 z-10 pointer-events-none">
                {hotspots.map((spot) => {
                  ...
                  const cardVertical = isTopHalf
                    ? "top-full mt-3"
                    : "bottom-full mb-3";
                  const arrowVertical = isTopHalf
                  ...
                  return (
                    ...
                    >
                      <div
                        className="relative -translate-x-1/2 -translate-y-1/2 group cursor-pointer p-2"
                        onClick={() =>
                      ...
                      >
                        <motion.div
                          ...
                          className={`absolute inset-2 rounded-full ${isDay ? "bg-amber-400" : "bg-blue-400"}`}
                        />
                        <div
                          className={`relative w-5 h-5 rounded-full border-2 border-white shadow-lg transition-colors duration-300 ${isActive ? (isDay ? "bg-amber-500" : "bg-blue-500") : "bg-white group-hover:bg-emerald-400"}`}
                        />
                      </div>
                      ...
                      <AnimatePresence>
                        {isActive && (
                          ...
                          >
                            <h3
                              className={`font-bold text-lg mb-1 ${cardText}`}
                            >
                              {spot.title}
                            </h3>
                            <p className={`text-sm leading-snug ${cardDesc}`}>
                              {spot.description}
                            </p>
                            <div
                              className={`absolute ${arrowVertical} ${arrowAlign} w-4 h-4 transform rotate-45 border-white/20 ${cardBg}`}
                            />
                          </motion.div>
                        )}
                      </AnimatePresence>
